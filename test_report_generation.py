...
def load_session_notes():
    ...
    if notes_file.exists():
        ...
        print("⚠️  Note: This test uses the master prompt without actual notes content")
        print(
            "⚠️  To process DOCX files, the full generator uses additional libraries\n"
        )
        return None
...
def main():
    ...
    test_prompt += (
        "Lightning talks data and attendees data will be provided from local CSV files."
    )
    ...
    try:
        ...
        print("📊 Statistics:")
        print(
            f"   ⏱️  Time elapsed: {timedelta(seconds=int(elapsed))} ({elapsed:.2f}s)"
        )
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("# Test Report Generation Output\n")
