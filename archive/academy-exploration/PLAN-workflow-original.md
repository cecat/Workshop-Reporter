...
**Architecture:** LangGraph workflow with tested components from TPC-Session-Reporter prototype
...
> **Why LangGraph**: After evaluating Academy (federated scientific computing middleware), we determined it's architectural overkill for a document processing pipeline. LangGraph is purpose-built for LLM workflows and provides the orchestration we need without unnecessary complexity.
...
### Runtime style
- **LangGraph workflow**: stateful graph with explicit nodes, conditional edges, and checkpointing
- **Not using**:
