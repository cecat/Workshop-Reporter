...
def parse_lightning_talks():
    ...
    talks = []
    with open("test_downloads/sheet1.csv") as f:
...
def parse_attendees():
    """Parse attendees CSV."""
    attendees = []
    with open("test_downloads/sheet2.csv") as f:
        reader = csv.DictReader(f)
        for row in reader:
            attendees.append(
                {
...
def read_notes():
    ...
    with open("test_downloads/doc1.txt") as f:
        return f.read().strip()
    ...
    for talk in lightning_talks:
        for author in talk["authors"]:
            if author["name"] and author["name"] not in attendee_names:
                attendees.append(
                    {"name": author["name"], "organization": author["affiliation"]}
                )
